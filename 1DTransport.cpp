...
void Trasport_coefs_calc(int N,int Nion,double *Ni,double *Del,double *Muel,double *Di,double *Mui,double *Lam,double *Pgas,double *Tgas,double *Te,bool EqP)
{
    ...
    for(n=0;n<N;n++)
    {
        for(i=0;i<=LEN+1;i++)
        {
            ...
            ///Lam = 1.0e+7*(2.714+5.897e-3*Tin)*1.0e-4; //[Ёрг/(с*см*K)]Ref. - Nuttall_JRNBS_1957 (N2)
            if(n==N-1)
                Lam[i] = 34*pow(Tgas[i],0.76);//[Mankelevich_form_consists_with_—правочник-‘из¬еличин]
        }
    }
}
...
void TeTransport(double *Ne,double *Te,double *Lel,double *Vel,int Nal,double dt)
{
    ...
    */
    ...
    //проверить_геометрические факторы в поправке дл€ сноса!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
